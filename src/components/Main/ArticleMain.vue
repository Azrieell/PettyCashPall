<template>
  <h1 class="md:text-4xl text-3xl font-bold mt-24 text-center">Financial Insights</h1>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-2">
    <!-- Card 1 -->
    <div v-for="(article, index) in getArticle" :key="index">
      <router-link :to="{ name: 'SingleArticleMain', params: { id: article.id } }">
      <div class="relative flex min-h-screen flex-col justify-center overflow-hidden py-6 sm:py-12">
        <div
          ...
          <div class="relative z-10 mx-auto max-w-md">
            ...
            <div
              class="space-y-6 pt-5 text-lg leading-7 text-black transition-all duration-300 group-hover:text-white/90">
              <p>{{ article.title }}</p>
            </div>
            <div
