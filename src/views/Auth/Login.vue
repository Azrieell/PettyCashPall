                        ...
                        <div class="flex -mx-3">
                            <div class="w-full px-3 mb-5">
                                ...
                                <div class="flex">
                                    ...
                                    <input type="email" id="email" v-model="formKey.email"
                        ...
                        <div class="flex -mx-3">
                            <div class="w-full px-3 mb-5">
                                ...
                                <div class="flex">
                                    ...
                                    <input type="password" id="password" v-model="formKey.password"
                                        class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500"
                                        placeholder="Password">
                                </div>
                            </div>
                        </div>
                        <p class="mb-12 text-xs flex justify-end">Need an account?
                            <router-link to="/auth/register">
                                <a href="#" class="text-blue-500 hover:text-blue-700 font-semibold ml-1">Create an
                                    account</a>
                            </router-link>
                        </p>
                        <div class="flex -mx-3">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async loginUser() {
                try {
                    // Panggil action login dengan data email dan password
                    await this.login(this.formKey);

                    // Setelah login berhasil, periksa peran pengguna
                    const userRole = this.$store.getters["auth/getMe"].role;
                    localStorage.setItem("role", userRole);
                    this.$store.commit("auth/SET_ROLE", userRole);
                    ...
                    // Tampilkan pesan error jika terjadi kesalahan saat login
                    if (error.response && error.response.status === 401) {
                        // Jika password salah, tampilkan pesan dari respons error
